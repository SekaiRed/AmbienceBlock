...
public class AmbienceController {
    ...
    @SubscribeEvent
    public void leaveWorld(FMLNetworkEvent.ClientDisconnectionFromServerEvent e) {
        compendium.clear();
        clear();
    }
    ...
    private void systemTick() {
        ...
        delayList.removeIf(delay -> (delay.getSource() instanceof AmbienceTileEntity) && !mc.world.loadedTileEntityList.contains((AmbienceTileEntity) delay.getSource()));
    ...
    private void renderInvisibleTileEntityParticles() {
        ...
        for(AmbienceTileEntity tile : tiles) {
            if(holdingBlock && tile.getBlockType().equals(ModBlocks.INVISIBLE_AMBIENCE_BLOCK)) {
                mc.effectRenderer.addEffect(new AmbienceParticle(mc.world, tile.getPos().getX() + 0.5D, tile.getPos().getY() + 0.5D, tile.getPos().getZ() + 0.5D, Item.getItemFromBlock(ModBlocks.INVISIBLE_AMBIENCE_BLOCK)));
            }
        }
    }
    ...
    public void stopFromTile(AmbienceTileEntity tile) {
        ...
        delayList.removeIf(delay -> delay.getSource() == tile);
    }
    ...
    public AmbienceDelayRestriction getDelayEntry(IAmbienceSource origin) {
        for(AmbienceDelayRestriction restriction : delayList) {
            if(restriction.getSource() == origin) return restriction;
        }
        return null;
    }
