...
public class PlayerPosWithinRegionCond extends AbstractCond {
    ...
    public boolean isTrue(EntityPlayer player, World worldIn, IAmbienceSource sourceIn) {
        if(AmbienceWorldSpace.ABSOLUTE.equals(space))
            return equal.testFor(boundingBox.contains(getPlayerPos(player).toVec3d()));
        else
            return equal.testFor(boundingBox.contains(getPlayerPos(player).subtract(sourceIn.getOrigin()).toVec3d()));
    }
    ...
    public NBTTagCompound toNBT() {
        ...
        nbt.setTag(START, NBTHelper.writeVec3d(getStartPos()));
        nbt.setTag(FINAL, NBTHelper.writeVec3d(getFinalPos()));
    ...
    public void fromNBT(NBTTagCompound nbt) {
        Vector3d startPos = NBTHelper.readVec3d((NBTTagCompound) nbt.getTag(START));
        Vector3d finalPos = NBTHelper.readVec3d((NBTTagCompound) nbt.getTag(FINAL));
        xS = startPos.x;
        yS = startPos.y;
        zS = startPos.z;
        xF = finalPos.x;
    ...
    private void updateBoundingBox() {
        //boundingBox = new AxisAlignedBB(getStartPos().toVec3d(), getFinalPos().toVec3d());
        Vector3d sPos = getStartPos();
        Vector3d fPos = getFinalPos();
        boundingBox = new AxisAlignedBB(sPos.x, sPos.y, sPos.z, fPos.x, fPos.y, fPos.z);
    }
