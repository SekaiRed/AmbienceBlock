...
public class CondsUtil {
    //nbt stuff
    public static NBTTagCompound toNBT(AbstractCond cond) {
        NBTTagCompound compound = cond.toNBT();
        compound.setString("condName", cond.getName()); //put type information
        return compound;
    }

    public static AbstractCond fromNBT(NBTTagCompound nbt) {
    ...
    public enum CondList {
        ...
        PLAYER_BIOME(109, new PlayerBiomeCond(AmbienceEquality.EQUAL_TO, Biomes.PLAINS.getRegistryName().toString())),
        PLAYER_DIMENSION(110, new PlayerDimensionCond(AmbienceEquality.EQUAL_TO, DimensionType.OVERWORLD.getName())),
        PLAYER_STRUCTURE(111, new PlayerStructureCond(AmbienceEquality.EQUAL_TO, StructureUtil.getListOfStructureTypes().get(0), 0, true)),
